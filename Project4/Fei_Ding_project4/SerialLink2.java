...
class SerialLink2 {
  ...
  private static double piBound(double theta) {
...
//
    ...
    double[][] J_1 = inverseJacobian3(q);
    double[] Et = new double[3];
    Et[0] = xd - tool.x();
    Et[1] = yd - tool.y();
    Et[2] = piBound(theta_d - tool.theta());
    double [] prod = new double[J_1.length];
    for (int i = 0; i < prod.length; i++) {
      double sum = 0;
      for (int j = 0; j < J_1[0].length; j++) {
        sum += J_1[i][j] * Et[j];
      }
      prod[i] = sum;
    }
